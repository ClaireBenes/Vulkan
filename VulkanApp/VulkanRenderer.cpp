...
void VulkanRenderer::createGraphicsPipeline()
{
	...
	*/
	...
	// -- PIPELINE LAYOUT --
	array<vk::DescriptorSetLayout, 2> descriptorSetLayouts{ descriptorSetLayout, samplerDescriptorSetLayout };
...
void VulkanRenderer::recordCommands(uint32_t currentImage)
{
	...
	for( size_t j = 0; j < meshModels.size(); ++j )
	{
		...
		commandBuffers[currentImage].pushConstants(pipelineLayout, vk::ShaderStageFlagBits::eVertex, 0, sizeof(Model), &modelMatrix);
		...
		for( size_t k = 0; k < model.getMeshCount(); ++k )
		{
			...
			array<vk::DescriptorSet, 2> descriptorSetsGroup{ descriptorSets[currentImage], samplerDescriptorSets[model.getMesh(k)->getTexId()] };
			...
			// Execute pipeline
			commandBuffers[currentImage].drawIndexed(static_cast<uint32_t>( model.getMesh(k)->getIndexCount() ), 1, 0, 0, 0);
		}
	}
...
void VulkanRenderer::createUniformBuffers()
{
	...
	for( size_t i = 0; i < swapchainImages.size(); ++i )
	{
		createBuffer(mainDevice.physicalDevice, mainDevice.logicalDevice, vpBufferSize, vk::BufferUsageFlagBits::eUniformBuffer,
...
void VulkanRenderer::createDepthBufferImage()
{
	...
	depthBufferImage = createImage(swapchainExtent.width, swapchainExtent.height, depthFormat, vk::ImageTiling::eOptimal,
...
int VulkanRenderer::createTextureImage(const string& filename)
{
	...
	transitionImageLayout(mainDevice.logicalDevice, graphicsQueue, graphicsCommandPool, texImage, vk::ImageLayout::eUndefined, vk::ImageLayout::eTransferDstOptimal);
	...
	transitionImageLayout(mainDevice.logicalDevice, graphicsQueue, graphicsCommandPool, texImage, vk::ImageLayout::eTransferDstOptimal, vk::ImageLayout::eShaderReadOnlyOptimal);
...
int VulkanRenderer::createTexture(const string& filename)
{
	...
	vk::ImageView imageView = createImageView(textureImages[textureImageLocation], vk::Format::eR8G8B8A8Unorm, vk::ImageAspectFlagBits::eColor);
